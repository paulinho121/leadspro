...
A plataforma coleta dados estritamente de **fontes públicas** (Google Maps, Bases de CNPJ, Redes Sociais Públicas). O tratamento desses dados segue o disposto no Art. 7º, §3º e §4º da LGPD, priorizando a transparência e a finalidade comercial legítima.

---

## 3. Coleta e Tratamento de Dados

### 3.1. Dados dos Leads Prospectados
Podemos processar e enriquecer os seguintes dados de empresas e seus respectivos sócios/decisores:
*   Informações Corporativas (Razão Social, Nome Fantasia, CNPJ, Endereço, CNAE).
*   Dados de Contato (Telefones comerciais, E-mails corporativos, Links de Redes Sociais).
*   Dados de Sócios e Administradores (Nomes e cargos, conforme Quadro de Sócios e Administradores - QSA público).

### 3.2. Finalidade do Tratamento
O tratamento de dados é realizado para:
1.  Viabilizar prospecção comercial B2B.
2.  Gerar "Attack Plans" (Planos de Ataque) via IA para otimizar vendas.
3.  Organizar o pipeline de vendas dos Usuários.
...
Implementamos rigorosos padrões de segurança "By Design":
*   **Isolamento de Dados (Multi-tenancy):** Cada usuário possui sua base de dados isolada via *Row Level Security* (RLS) no Supabase.
*   **Criptografia:** Todos os dados são transmitidos via protocolos seguros (SSL/TLS).
*   **Gestão de Chaves:** Chaves de API de terceiros (Gemini, Serper) são armazenadas de forma segura e nunca expostas no front-end.
...
*   **Exclusão definitiva** de seus dados da plataforma.

Para solicitações de exclusão, o titular pode entrar em contato através do e-mail: **labwpplus@gmail.com**.
...
Reservamo-nos o direito de atualizar estes termos periodicamente. O uso continuado da plataforma constitui aceitação das novas versões. Fica eleito o foro da comarca de São Paulo/SP para dirimir quaisquer questões relativas a este contrato.
