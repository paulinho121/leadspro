    ...
    return (
        <div className="p-6 md:p-10 space-y-8 animate-fade-in max-w-7xl mx-auto h-full overflow-y-auto custom-scrollbar">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h2 className="text-3xl font-black text-white tracking-tighter uppercase italic">Revenue Automação</h2>
                    <p className="text-slate-500 font-mono text-[10px] uppercase tracking-[0.2em] mt-1">Intelligent_Outreach_System</p>
                </div>
                <button
                    onClick={() => setShowNewModal(true)}
                    className="flex items-center gap-3 px-8 py-4 bg-primary text-slate-900 font-black text-xs uppercase tracking-widest rounded-2xl hover:scale-105 transition-all shadow-xl shadow-primary/20"
                >
                    <Plus size={18} /> Nova Cadência
                </button>
            </div>
                    ...
                    <div className="glass-strong max-w-2xl w-full p-10 rounded-[3rem] border border-white/10 shadow-2xl animate-in zoom-in-95 duration-300 relative overflow-hidden">
                        ...
                        <div className="space-y-10 relative z-10">
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-6">Nome da Estratégia de Receita</label>
                                <input
                                    autoFocus
                                    value={newSequence.name}
                                    onChange={e => setNewSequence(prev => ({ ...prev, name: e.target.value }))}
                                    placeholder="Ex: Aquisição - Varejo SP - Alta Prioridade"
