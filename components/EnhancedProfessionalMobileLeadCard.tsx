...
interface EnhancedProfessionalMobileLeadCardProps {
  lead: Lead;
...
// Componente de Informações Principais (Expandido)
const CompanyHeader: React.FC<{ lead: Lead }> = ({ lead }) => {
  ...
  const getCompanySize = (size?: string) => {
    ...
    const sizeMap: { [key: string]: { label: string; color: string } } = {
      'micro': { label: 'Microempresa', color: 'text-blue-400' },
      'pequeno': { label: 'Pequena', color: 'text-green-400' },
      'medio': { label: 'Média', color: 'text-amber-400' },
      'grande': { label: 'Grande', color: 'text-red-400' },
      'enorme': { label: 'Grande Porte', color: 'text-purple-400' }
    };
    ...
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {companySize && (
            <div className={`flex items-center gap-1 px-2 py-1 rounded-lg ${companySize.color} bg-opacity-10 bg-current`}>
              <Building size={10} />
              <span className="font-medium text-xs">{companySize.label}</span>
            </div>
          )}
          
          {employeeCount && (
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-700/50 text-slate-300">
              <Users2 size={10} />
              <span className="font-medium text-xs">{employeeCount} func.</span>
            </div>
          )}
        </div>
      </div>
...
const PartnersSection: React.FC<{ lead: Lead }> = ({ lead }) => {
  const partners = lead.details?.partners || lead.details?.qsa || [];
  
  if (!partners || partners.length === 0) return null;

  const displayPartners = partners.slice(0, 3); // Mostrar até 3 sócios
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {displayPartners.map((partner: any, index: number) => (
          <div key={index} className="flex items-start gap-3 p-2 rounded-lg bg-slate-800/30 border border-slate-600/20">
        ...
        {hasMore && (
          <div className="flex items-center justify-center py-2 px-3 rounded-lg bg-slate-700/50 border border-slate-600/20">
            <span className="text-xs text-slate-400">
              +{partners.length - 3} outros sócios
            </span>
          </div>
        )}
      </div>
...
const MobileActions: React.FC<{ lead: Lead; actions: EnhancedProfessionalMobileLeadCardProps }> = ({ lead, actions }) => {
...
export const EnhancedProfessionalMobileLeadCard: React.FC<EnhancedProfessionalMobileLeadCardProps> = (props) => {
  const { lead } = props;
